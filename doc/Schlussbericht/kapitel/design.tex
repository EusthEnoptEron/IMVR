...
Bei der Entwicklung wurden diverse Design-Entscheidungen gefällt, welche zum Teil bereits im Vorprojekt analysiert wurden. Diese sollen in diesem Kapitel aufgelistet und erläutert werden.

\section{Systemübersicht}
\label{sec:sysoverview}

Das Zusammenspiel der Oculus Rift und der Leap Motion ist auf den ersten Blick nicht ganz offensichtlich. Erstere wird aufgesetzt und verdeckt die Sicht, letztere wird auf den Schreibtisch gestellt und erkennt Hände über sich.
...
Die Komponenten sind erklärt und es sollte nun klar sein, wie die Hardware miteinander verknüpft ist. Das Kernproblem bzw. die Aufgabe dreht sich jedoch um die \textit{Software} dazu. Diese soll an dieser Stelle in den Vordergrund treten.
...
\begin{figure}[h]
	...
	\caption{Überblick über den Programmablaufs}
	\label{fig:flow}
\end{figure}

Wie in der Aufgabenstellung erwähnt, besteht das Projekt aus zwei Teilen: einem Teil zur Indexierung und einem interaktiven Teil. Diese Gliederung zieht sich durch das ganze Design der Applikation durch. So wird beispielsweise die Indexierung in einem komplett abgespaltenen Bereich durchgeführt, und gilt somit als eigenes Programm.
...
Man erkennt wie der Benutzer zuerst ausserhalb der Unity-Applikation den Indexer startet und durchlaufen lässt. Dieser durchläuft alle Ordner, die er in der Datei \textit{library.conf} findet und schreibt diese in die Datenbank. Im nächsten Schritt werden aus diversen Quellen weitere Metadaten zusammengetragen.
...
Um besser zu verstehen, wo die einzelnen Komponenten platziert sind, soll an dieser Stelle kurz die Ordnerstruktur erläutert werden.
...
Alle Quellcode-Dateien sind im Ordner \textit{src} abgelegt. Die Projektdateien für die Applikation (IMVR) und den Indexer sind jedoch voneinander getrennt und verfügen jeweils über ihre eigene Struktur.

Der Unity-Teil trennt externe Scripts und Plugins explizit von eigens geschriebenen. 

Der Indexer befindet sich im Ordner bzw. in der Solution \textit{AuxiliaryTools}, weil er eine unterstützende Funktion hat. Am gleichen Ort findet sich auch ein SpeechServer, der im Vorprojekt entwickelt wurde und in einem weiteren Schritt für die Implementierung einer ergänzenden Sprachfunktion in IMVR dienen könnte.
...
Da der Programmfluss nun klar geworden ist, bedarf es einer genaueren Erklärung der einzelnen Komponenten im System. Aufgrund der Brückenfunktion und der somit globalen Relevanz, wird zuerst ein Blick auf die Klassenstruktur der Datenbank geworfen. Danach wird der Aufbau des Indexers untersucht, und zuletzt schliesslich die eigentliche Applikation.
...
\afterpage {
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{ l X }
			...
			IMDB\footnote{Es besteht keinerlei Verbindung zur Internet Movie Database.} & Zentraler Zugangsknoten auf die Daten. Hält Referenzen auf die Artisten, Songs und Atlasse. \\
			\bottomrule
		\end{tabularx}
...
Es gäbe keine Grenzen, wenn man eine perfekte Struktur erreichen wollte, aber das liegt ausserhalb der angestrebten Funktionalität dieses Projektes.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		Options & Singleton-Klasse, welche mit den Command-Args gebildet wird und die Konfigurationen beinhält.\\ \midrule
		...
		AbstractWorker & Jede Node erbt von dieser Klasse. Sie sorgt dafür, dass die Nodes auf eine einheitliche Weise initialisiert, durchgeführt und aufgeräumt werden können.\\ \midrule
		...
		DualNode & Hat die gleiche Funktion wie eine \code{ConsumerNode}, aber implementiert zusätzlich \code{IProducer} und bietet mit \code{Publish} ebenfalls eine Hilfsmethode an.\\ \midrule
...
Die Hauptapplikation bedarf ebenfalls einer Erklärung. Um das Design besser verdaulich zu machen, wurde hierbei das Schema in verschiedene, logische Teilbereiche gegliedert, die auch den Namespaces des Projekts entsprechen.

Es wird empfohlen, diese Schemas zusammen mit den Erklärungen in Kapitel \ref{chap:implementation_imvr} zu verwenden, da diese eng miteinander verknüpft sind.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		ModeController & Führt Buch über den momentan aktiven Modus.\\ \midrule
		Mode & Verfügt über einen eigenen Stack von Views, durch die der Anwender navigiert.\\ \midrule
		BrowseMode & Modus, in dem der Anwender gezielt durch seine Musiksammlung navigiert, indem ihm eine Übersicht aller Künstler angezeigt wird. Beginnt mit der View \code{ArtistOverView}.\\ \midrule
		ListenMode & Modus, in dem der Anwender nur die Art der Musik angibt, die er hören will. Die Applikation übernimmt den Rest. Beginnt mit der View \code{SelectorView}.\\ \midrule
		...
		ArtistOverView & Stellt einen gruppierten Zylinder mit den Künstlern in der Musiksammlung dar. \\ \midrule
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		RingMenu & Die zentrale Klasse des Music Arms und gleichzeitig auch die Root-Ebene des Menüs. Existiert nur einmal, verwaltet, welche Stufe momentan angezeigt wird, und sorgt für das Anzeigen und Verstecken des Menüs.\\ \midrule
		RingMenuItem & Ein einzelner Menüeintrag im Menü. Platziert sich m den entsprechenden Finger wenn sichtbar und hat einen Event-Handler für Click-Events (in diesem Fall Auswahl des Menüs).\\ \midrule
		IRingMenu & Interface für Menüs mit Einträgen.\\ \midrule
		...
		RingMenuBuilder & Hilft beim dynamischen Erstellen von neuen Menüeinträgen.\\ \midrule
		...
		PlaylistView & Zeigt die momentan Playlist an und befindet sich auf der Rückseite des Arms.\\ \midrule
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		RingActivator & Abstrakte Klasse, die visuell von einem sich füllenden Ring-Mesh dargestellt wird, und beim anhaltenden Blick aktiviert wird.\\ \midrule
		ModeActivator & Eine Unterklasse des \code{RingActivator}s, die nur dazu zuständig ist, einen Modus zu aktivieren (siehe \textit{IMVR.Interface}). \\ \midrule
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		CylinderLayout & Komponente für die allgemeine Darstellung von GameObjects in einem Zylinder-Layout. Kann per \code{CylinderInteractor} rotiert werden und verfügt über einen optimierten Raycaster.\\ \midrule
		...
		CylinderInteractor & Kümmert sich um die User-Interaktion mit dem Layout. Hilft beim Rotieren und vertikalen Scrollen. \\ \midrule
		CylinderRaycaster & Sorgt für ein effizientes Raycasting der Elemente im Layout. Wird wichtig, wenn über 100 interaktive Elemente im Zylinder verteilt sind.\\ \midrule
...
Der Teil, der für die Bilder zuständig ist, enthält drei Klassen: eine Helper-Klasse, die auf einer hohen Schicht für das einfache Laden von Atlassen aus IMVR.Commons sorgt, eine intelligente Klasse zum Laden von Bildern und eine tiefe Klasse, welche direkt mit den Daten der Atlasse arbeitet.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ l X }
		\toprule
		\textbf{Klasse} & \textbf{Beschreibung} \\ \midrule
		AtlasHelper & Hilft beim Laden von Atlassen aus der Datenbank, indem er logische Methoden fürs Laden liefert. Gibt zudem Auskunft über die aktiven Ladevorgänge.\\ \midrule
		DeferredLoader & Lädt und verwaltet Atlasse vom Filesystem, und liefert Texturen und Sprites für den Zugriff darauf.\\ \midrule
...
Im Namespace für die Musik gibt es zwei Arten von Klassen: die Klassen, welche um die Jukebox herum gebaut sind, und die Klassen, welche zur Darstellung von Musikdaten dienen.
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ l X }
		...
		Playlist & Eine Liste von Songs, die für die Wiedergabe vorgesehen sind. Hat eine momentane Selektion, die geändert werden kann. Sender Events beim Ändern der Liste bzw. der momentanen Selektion.\\ \midrule
		SongItem & Zeigt ein Lied an. \\ \midrule
		PlaylistSongItem & Zeigt ein Lied an, optimiert für die Anzeige in \code{PlaylistView}.\\ \midrule
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{ l X }
		...
		SpectrumBar & Ein Mesh, welches je nach Stärke des Signals in einem bestimmten Frequenzbereich, höher steigt bzw. tiefer sinkt.\\ \midrule
		RingVisualizer & Eine kreisförmige, kontinuierliche Linie, welche die Lautstärke eines Kanals (links oder rechts) darstellt.\\ \midrule
		BeatDetector & Klasse zur Erkennung von Beats in der wiedergegebenen Musik.\\ \midrule
		Pulsator & Komponente, welche ein Objekt im Takt der Musik pulsieren lässt.\\ \midrule
		WaveEmitter & Sendet im Takt der Musik Wellen in entsprechender Stärke aus (siehe Kapitel \ref{subsec:waves})\\ \midrule
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{ l X }
		...
		FillableRing & Erweitert ein Ring-Mesh mit einer Füllung, die mit der Property \code{Fill} auf einen Wert im Bereich $[0..1]$ eingestellt werden kann.\\ \midrule
