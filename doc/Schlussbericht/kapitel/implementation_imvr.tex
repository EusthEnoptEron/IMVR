...
Dieses Kapitel dokumentiert die eigentliche Applikation \textit{IMVR}. Vieles hat sich im Laufe des Projekts verändert und entwickelt, und einige Probleme traten zum Vorschein, die an dieser Stelle genauer erläutert werden sollen.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{bilder/unity}
	\caption{Screenshot der Entwicklungsumgebung}
	\label{fig:unity}
\end{figure}
...
\begin{enumerate}
	\item Designen des UIs im Editor
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\linewidth]{bilder/aufbau}
	\caption{Aufbau der Szene}
	\label{fig:aufbau}
\end{figure}
...
\begin{enumerate}
	\item das Event-System von Unity (wird für den User-Input verwendet)
	\item die gesamte Welt (näher erläutert in Kapitel \ref{subsec:performance})
	\item die korreke Positionierung der Kamera (ebenfalls erläutert in Kapitel \ref{subsec:performance})
	\item die Handhabung der Partikel der Hände
\end{enumerate}
...
Von den Ressourcen her sieht der Aufbau so aus, dass alle visuellen bzw. akustischen Assets im ``Resources''-Ordner abgelegt werden. Unity erlaubt das Laden von Assets im Code nur, wenn diese in diesem Ordner zu finden sind. Der Nachteil ist, dass beim Kompilieren \textit{alle} Dateien innerhalb von \textit{Resources} für den Release gepackt werden - unabhängig davon, ob sie wirklich eingesetzt werden.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{bilder/screenshot_map}
	\caption{Vogelperspektive auf die Szene}
...
Implementiert wurden diese Fussplatten mit einem selbst erstellten Ring-Mesh (siehe Kapitel \ref{subsec:waves}), welches zur Laufzeit generiert werden kann und dynamisch genug ist, um animiert zu werden. Bei der Benutzung werden insgesamt 4 Klassen angewendet: zwei zur Darstellung und zwei für die Logik.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\linewidth]{bilder/ringmesh_collider}
	\caption{Aufbau einer Fussplatte}
	\label{fig:ringmesh_collider}
\end{figure}

In Abbildung \ref{fig:ringmesh_collider} sind diese nummeriert abgebildet. Die Rolle der Klassen sieht dabei folgendermassen aus:

\begin{table}[H]
	\centering
	\label{tab:ringmesh_aufbau}
	\begin{tabularx}{\textwidth}{ l l X }
		\toprule
		\textbf{\#} & \textbf{Komponente} & \textbf{Rolle} \\ \midrule
		1 & Ring Mesh & Sorgt für die Darstellung eines Ring-Meshes, indem ein Mesh anhand der angegebenen Parameter generiert wird.\\
		2 & Fillable Ring & Erweitert die Ring Mesh Komponente mit einem zweiten Kreissektor, der die gleichen Werte wie das Ring Mesh hat, bis auf den äusseren Radius. Dieser wird anhand des \code{Fill}-Wertes zwischen 0 und 1 verschoben.\\
		3 & Mode Activator & Reagiert auf Blicke und steuert den \code{Fill}-Wert des Fillable Rings.\\
		4 & Mode & Stellt den Modus dar, der vom Mode Activator aktiviert werden soll, sobald der Blick lange genug gehalten wurde.\\
		\bottomrule
	\end{tabularx}
	\caption{Beschreibung des Aufbaus der Fussplatten}
\end{table}

Um den Blick des Anwenders zu erkennen, wurde bei den Fussplatten jeweils an einer geeigneten Position ein Collider gesetzt (Nummer 5 auf Abbildung \ref{fig:ringmesh_collider}), der auf einer für die Fussplatten dedizierten Ebene liegt und per Raycasting geprüft wird.
...
\subsection{Zylinderinteraktion}
...
Um dieses Feature zu implementieren, fiel der Entscheid ebenfalls auf das UI-System von Unity, da dieses die nötigen Elemente (Text, Bilder, Widgets) liefert. Um die Interaktion zu ermöglichen, wurde ein selbst geschriebenes Input-Modul verwendet, welches die Selektierung, Aktivierung und Deaktivierung von UI-Elementen übernimmt. Wie Input-Module funktionieren, wird in der Vorarbeit genauer erläutert. Wichtig zu wissen ist, dass es diese auch für die Maus- und Tastatursteuerung sowie für Touchpads gibt. Input-Module eignen sich also für diese Funktionalität.
...
\begin{enumerate}
	\item Der Punkt $P_\textit{3D}$, wo sich die Fingerspitze befindet, wird durch die Event-Kamera (linkes Auge) in einen 2D Punkt $P_\textit{2D}$ umgewandelt.
	\item Ein Strahl wird durch $P_\textit{2D}$ in die Szene geworfen und gibt eine Liste von Schnittpunkten $S$ zurück.
	\item Die Schnittpunkte $S$ werden nach dem Element durchsucht, welches die niedrigste, rechtwinklige Distanz $d$ zu $P_\textit{3D}$ aufweist und wo gilt $d < T$, wobei $T$ ein Schwellwert ist.
	\item Das gefundene Element wird in die Input-Pipeline gesendet und weiterverarbeitet.
\end{enumerate}
...
Als Hilfe, damit besser erkannt wird, wann ein Klick geschehen kann, dient ein Fadenkreuz, welches kleiner wird, je näher der Finger der Schaltfläche kommt. Die Idee stammt aus der Leap Motion Demo \textit{Soundscape VR}\footnote{https://apps.leapmotion.com/apps/soundscape-vr/windows}, welche Fadenkreuze für die weit entfernten Objekte verwendet.
...
\begin{figure}
	\centering
	\frame{\includegraphics[width=0.7\linewidth]{bilder/hand1}}
...
\begin{enumerate}[label=(\alph*)]
	\item Die Bone-Weights werden auf der GPU angewendet, deshalb muss das Mesh in jedem Frame auf der CPU nachgebildet werden.
...
Im Rahmen dieses Projekts wurde ein Point-Chart entwickelt, der dazu dient, die Musik des Anwenders in ein dreidimensionales System einzuordnen und auf einen Blick darzustellen. Die Dimensionen sind 1:1 die Features, die während der Indexierung von The Echo Nest heruntergeladen wurden, also Merkmale wie \textit{Danceability}, \textit{Energy} und \textit{Tempo} (siehe Kapitel \ref{sec:design_commons}).
...
Wie auf dem UML (Abbildung \ref{uml:IMVR.FX}) ersichtlich, wurde die Implementation des Point Charts in drei verschiedenen Klassen durchgeführt.
...
Um die gewonnenen Daten zentral zu verwalten, wurde nun eine Klasse namens \code{VisualizationHelper} erstellt. Diese hilft ausserdem dabei, die momentane Lautstärke der Stereo-Kanäle zu bestimmen, indem sie ein \gls{rms}-Wert über alle Samples erstellt \cite{outputData}.
...
label={lst:spectrumbar}]{code/SpectrumBar_Update.cs}
...
Stattdessen wurde auf eine andere C\#-Bibliothek ausgewichen: CSCore \cite{cscore}. Im Gegenzug zu einer schlechteren Portabilität als NAudio (läuft nur auf Windows) bietet CSCore Klassen zur einfachen Konversion von codierten Audio-Streams zu simplen PCM-Streams an.
...
Canvas-Elemente, welche jeweils an oberster Stelle eines uGUIs stehen, werden als ganze Meshes betrachtet, und seriell gerendert. Überschneidungen mit gewöhnlichen Meshes werden zwar korrekt gehandhabt, schneiden sich aber zwei Canvas-Elemente führt dies zu unerwünschten Ergebnissen (Abbildung \ref{fig:canvas_z}).

\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\linewidth]{bilder/canvas_z}
	\caption{Überschneidung zweier Canvas-Elemente}
	\label{fig:canvas_z}
\end{figure}
...
Mit ``komplexen Canvas'' sind Canvas-Elemente gemeint, die tief reichend sind und deren Unterelemente eine Positionierung auf der Z-Achse (``in die Tiefe'') besitzen. Ein Canvas ist entweder hinter einem anderen Canvas oder davor. Wenn jedoch Unterobjekte mit unterschiedlichen Tiefen existieren, kommt es zu Fällen, wo manche Objekte hinter anderen Canvas liegen und andere weiter vorne. Abbildung \ref{fig:canvas_z_2} stellt dieses Verhalten bildlich dar.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\linewidth]{bilder/canvas_z_2}
	\caption{Probleme mit dem Rendering von komplexen Canvas}
	\label{fig:canvas_z_2}
\end{figure}

Ursprünglich wurden versucht, bei der Erstellung von UI-Elementen möglichst wenige Canvas zu benutzen, um einen Overhead zu vermeiden. Dieses Vorgehen stellte sich jedoch aufgrund des oben erwähnten Problemes als Fehler heraus, denn die Rendering-Reihenfolge war dadurch beinahe willkürlich.
...
Eine Ausnahme dazu stellt das ``Dial''-Menü in der Künstlerübersicht dar. Dieses hätte stark überarbeitet werden müssen, was zeitlich nicht mehr reichte. Ausserdem wäre dadurch die Anzahl der benötigten Draw-Calls wieder drastisch gestiegen, da Unity Canvas-Elemente nicht \textit{batcht} (sprich, die Draw-Calls nicht zusammennimmt).

%TODO: Alpha-Sorting, etc. Tile-Blanket
...
\begin{enumerate}
	\item Benutzen der Low-level Native Plugin API von Unity
	\item Verwenden von kleinen Texturen
	\item Pre-Loaden der Texturen
\end{enumerate}

Die erste Möglichkeit, die auf einer tiefschichtigen API von Unity basiert \cite{unity_lowlevel}, hätte eventuell zu einer Lösung geführt, doch die Unsicherheit einer Lösung und die ungenügende Dokumentation des Features hat schliesslich zu einem Gegenentscheid geführt.

Das Benutzen von kleinen Texturen bringt wiederum andere Probleme mit sich. Zum einen ist es dann nicht mehr möglich qualitativ hochwertige Bilder anzuzeigen, andererseits müsste auf Atlasse verzichten werden, was zu \textit{vielen} Draw-Calls führen würde. Performance-technisch wäre dies also keine Lösung.

Schliesslich fiel der Entscheid auf die letzte Variante - das Pre-Loading. Dafür wurde ein Ladebildschirm implementiert, der mithilfe von \code{AtlasHelper.Progress} (siehe Abbildung \ref{uml:IMVR.Data}), prüft, wie viel geladen wurde, und wann er sich wieder verstecken kann.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{bilder/loadingscreen}
	\caption{Aufbau des Ladebildschirms}
	\label{fig:loadingscreen}
\end{figure}

Implementiert ist das mit einem zweiten Set von Kameras, die \textit{über} die vorhandene Szene rendern, und jederzeit bei aufwendigen Operationen aktiviert werden können. Um das Umzusetzen, benötigt es ein paar spezielle Einstellungen in der Kamera (vgl. mit Abbildung \ref{fig:loadingscreen}):

\begin{enumerate}
	\item Das vorherig gerenderte muss mit der Farbe Schwarz überschrieben werden und die Culling-Maske so eingestellt werden, dass nur die \textit{Overlay}-Ebene gerendert wird.
	\item Die Tiefe der Kameras muss höher eingestellt werden, als die eigentlichen Kameras. Das ist notwendig, damit der Ladebildschirm \textit{nach} dem Rest gerendert wird.
\end{enumerate}

Der Ladebalken selbst wird mithilfe der \code{Fill}-Property von Bildelementen gefüllt. Das Feature ist tief im UI-System von Unity verbaut und eignet sich perfekt für diese Anwendung. Damit der Ladebildschirm an korrekter Stelle gerendert wird, wurde das Objekt der Kamera angehängt und bewegt sich daher mit.

Im Verlauf der Entwicklung stellte sich ebenfalls heraus, dass die Generierung der \textit{Font}-Texturen relativ zeitaufwendig ist. Deshalb werden im Loader ebenfalls Fonts vorinitialisiert.

Was sich allerdings mit Abstand als am Performance-lastigsten herausstellte, war das generieren von Canvas-Meshes. Dies ist eine grundlegende Limitierung von Canvas-Elementen und wird eventuell in zukünftigen Versionen verbessert. Das Problem ist, dass Canvas-Elemente zu einzelnen Meshes zusammengefasst werden und dieser Vorgang sehr aufwendig ist. Das geschieht bei jeder noch so kleiner Änderung in der Canvas-Hierarchie - selbst bei einer Änderung der Position \cite{buildbatch}. Wenn der Anwender also die Zylinderansicht rotierte, führte dies zu einem enormen Performance-Loch.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\linewidth]{bilder/buildbatch}
	\caption{Performance-Einbussen bei \code{BuildBatch()}}
	\label{fig:buildbatch}
\end{figure}

Als Workaround um dieses Problem wurde beim Rotieren des Zylinders nicht der Zylinder rotiert, sondern die Welt. Das hat zur Folge, dass der Zylinder selbst unberührt bleibt und kein \code{BuildMesh()}-Call ausgelöst wird. Leider hat diese Lösung sehr grosse Einschränkungen: Physik wird nicht mehr funktionieren, weil sich die Welt immer wieder bewegt, auch äussere Zylinderschichten müssen sich mitdrehen, da sonst das gleiche Problem auftritt und alle Elemente müssen umständlich in einem Transform, der alle Welt-Objekte beinhält, organisiert werden.
...
Neben der Performance-Einbussen existierender Elemente, traten auch Probleme beim \textit{Initialisieren} von Objekten auf. Die Schwierigkeiten wurden sichtbar bei der Implementierung des Interfaces für die Playlist.

Beim \textit{Listen}-Modus kommt es oft vor, dass eine sehr grosse Anzahl Lieder einem Kriterium gerecht werden. Da jede Zeile in einer Liste in uGUI ein eigenes GameObject ist, hat dies zur Folge, dass entsprechend viele GameObjects \textit{on-the-fly} generiert werden müssen. Selektiert der Anwender also 1000 Songs, werden 1000 GameObjects erstellt, was zu einem enormen Einfall der Framerate führt.

Eine Lösung dafür bietet eine Komponente namens \textit{PooledList} \cite{pooled-list}, welche immer nur eine kleine Anzahl Objekte erstellt und lediglich den Inhalt mit den aktuell sichtbaren austauscht.

Leider ist das Projekt nicht sehr gut dokumentiert und baut auf einem \code{GridLayout} auf. In IMVR wird jedoch ein \code{VerticalLayoutGroup} verwendet, welches nicht mit dem Plugin kompatibel ist. Ein Versuch, die Playlist in ein Grid-Layout zu konvertieren schlug fehl.

Als Lösung wurde schliesslich selbst eine sehr simple, ähnliche Komponente entwickelt: die \code{LazyList}. Diese akzeptiert eine Liste von \code{LazyGameObjects} - eine selbst erstellte Klasse, die auf Anfrage mithilfe eines Callbacks, der im Konstruktor übergeben wird, jeweils ein GameObject erstellt - und nutzt diese, um das vertikale Layout zu füllen. Dabei werden zwei Tricks angewendet:

\begin{enumerate}
	\item Nur die aktuell sichtbaren Elemente werden angezeigt. Der Rest wird in einem deaktivierten GameObject zwischengelagert und vollständig ignoriert.
	\item Alle Elemente werden nur auf Anfrage initialisiert. Das heisst, dass anfangs kein einziges Objekt existiert und diese Schritt für Schritt beim Scrollen geladen werden.
\end{enumerate}

Mit diesen zwei Verbesserungen ist es nun möglich, sofort eine beliebig grosse Liste zu erstellen. Der Nachteil ist, dass irgendwann der Speicher ausgehen wird, da momentan nicht mehr angezeigte Objekte nicht aufgeräumt werden. Ein ähnlicher Ansatz wie bei der \textit{PooledList} wäre ideal.
